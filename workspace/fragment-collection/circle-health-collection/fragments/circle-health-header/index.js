...
(function() {
    ...
    try {
        ...
        if (!root) {
            const sigmaHeaders = document.querySelectorAll('[data-fragment-entry-key*="ch-header"]');
        ...
        if (!root) {
            const headerElements = document.querySelectorAll('.ch-header-container, .ch-header');
        ...
        if (!root) {
            ...
            for (const fragment of allFragments) {
                if (fragment.querySelector('.ch-nav, .ch-logo, .ch-header-container')) {
                    root = fragment;
                    break;
                }
            }
        ...
        if (!root) {
            console.warn('CIRCLE HEALTH HEADER');
            return;
        }
    ...
    function safeInitializeHeader() {
        ...
        initializeHeader();
        isInitialized = true;
        fragmentElement.setAttribute('data-ch-initialized', 'true');
    }
    ...
    function initializeHeader() {
        ...
        if (editMode) {
            ...
            try {
                ...
                const megaMenuContainer = fragmentElement.querySelector('.ch-mega-menu-dropzones');
    ...
    function isInEditMode() {
        ...
        if (inEditMode && !fragmentElement.classList.contains('ch-edit-mode')) {
            body.classList.add('has-edit-mode-menu');
            fragmentElement.classList.add('ch-edit-mode');
        } else if (!inEditMode && fragmentElement.classList.contains('ch-edit-mode')) {
            body.classList.remove('has-edit-mode-menu');
            fragmentElement.classList.remove('ch-edit-mode');
        }
    ...
    function ensureModalsHidden() {
        ...
        const searchOverlay = document.querySelector('#ch-search-overlay');
        const loginOverlay = document.querySelector('#ch-login-overlay');
        const languageDropzone = fragmentElement.querySelector('.ch-language-selector-dropzone');
        const accountDropzone = fragmentElement.querySelector('.ch-account-selector-dropzone');
        
        if (searchOverlay) {
            searchOverlay.classList.remove('ch-edit-mode');
            searchOverlay.style.display = 'none';
        }
        
        if (loginOverlay) {
            loginOverlay.classList.remove('ch-edit-mode');
            loginOverlay.style.display = 'none';
        }
        
        if (languageDropzone) {
            languageDropzone.classList.remove('ch-edit-mode');
        }
        
        if (accountDropzone) {
            accountDropzone.classList.remove('ch-edit-mode');
        }
    }
    ...
    function applyConfiguration(config) {
        const header = fragmentElement.querySelector('.ch-header');
        const searchBtn = fragmentElement.querySelector('.ch-search-btn');
        const userProfileWidget = fragmentElement.querySelector('.ch-user-profile-widget');
        const loginBtn = fragmentElement.querySelector('.ch-login-btn');
    ...
    function getSampleNavigation() {
        return [
            { name: 'Find a specialist', url: '/consultants' },
            { name: 'Find a hospital', url: '/hospitals', children: [{ name: 'Placeholder', url: '#'}] },
            { name: 'Find a treatment', url: '/treatments', children: [{ name: 'Placeholder', url: '#'}] },
            { name: 'Find a GP', url: '/gp' },
            { name: 'Going private', url: '/going-private', children: [{ name: 'Placeholder', url: '#'}] },
            { name: 'Pay my bill', url: '/pay-my-bill' }
        ];
    }
    ...
    function renderNavigation(navItems) {
        const navList = fragmentElement.querySelector('#ch-main-nav');
        const mobileNavList = fragmentElement.querySelector('.ch-mobile-nav-list');
    ...
    function createNavItem(item, index) {
        ...
        li.className = 'ch-nav-item';
        
        const link = document.createElement('a');
        link.href = item.url;
        link.className = 'ch-nav-link';
        ...
        if (item.children && item.children.length > 0) {
            ...
            dropdown.className = 'ch-dropdown-menu';
            
            item.children.forEach(child => {
                const childLink = document.createElement('a');
                childLink.href = child.url;
                childLink.className = 'ch-dropdown-item';
    ...
    function createMobileNavItem(item) {
        const li = document.createElement('li');
        li.className = 'ch-mobile-nav-item';
        
        const link = document.createElement('a');
        link.href = item.url;
        link.className = 'ch-mobile-nav-link';
        link.textContent = item.name;
        ...
        if (item.children && item.children.length > 0) {
            ...
            childrenContainer.className = 'ch-mobile-nav-children';
    ...
    function initializeDropdowns() {
        const dropdownItems = fragmentElement.querySelectorAll('.ch-nav-item.has-dropdown');
        
        dropdownItems.forEach(item => {
            const link = item.querySelector('.ch-nav-link');
            const dropdown = item.querySelector('.ch-dropdown-menu');
            ...
            item.addEventListener('mouseenter', () => {
                ...
                const otherItems = fragmentElement.querySelectorAll('.ch-nav-item.has-dropdown.show');
                otherItems.forEach(otherItem => {
                    if (otherItem !== item) {
                        otherItem.classList.remove('show');
                        const otherDropdown = otherItem.querySelector('.ch-dropdown-menu');
                        if (otherDropdown) {
        ...
        document.addEventListener('click', (e) => {
            if (!e.target.closest('.ch-nav-item.has-dropdown')) {
                closeAllDropdowns();
            }
        });
    }
    ...
    function closeAllDropdowns() {
        const openItems = fragmentElement.querySelectorAll('.ch-nav-item.has-dropdown.show');
    ...
    function initializeMobileMenu() {
        const mobileToggle = fragmentElement.querySelector('.ch-mobile-menu-toggle');
        const mobileNav = fragmentElement.querySelector('.ch-mobile-nav');
        ...
        document.addEventListener('click', (e) => {
            if (!e.target.closest('.ch-mobile-menu-toggle') && !e.target.closest('.ch-mobile-nav')) {
                mobileNav.classList.remove('show');
                mobileToggle.setAttribute('aria-expanded', 'false');
            }
        });
    ...
    function initializeSearchModal() {
        const searchBtn = fragmentElement.querySelector('.ch-search-btn');
        const searchOverlay = document.querySelector('#ch-search-overlay');
        const closeSearchBtn = document.querySelector('#ch-close-search');
    ...
    function initializeLoginModal() {
        const loginBtn = fragmentElement.querySelector('.ch-login-btn');
        const mobileLoginBtn = fragmentElement.querySelector('.ch-mobile-login-btn');
        const loginOverlay = document.querySelector('#ch-login-overlay');
        const closeLoginBtn = document.querySelector('#ch-close-login');
    ...
    function initializeLogoLink() {
        const logoLink = fragmentElement.querySelector('.ch-logo-link');
        const config = getFragmentConfiguration();
    ...
    function setupMegaMenuMappingWithRetry(attempt = 1, maxAttempts = 5) {
        ...
        const navItems = fragmentElement.querySelectorAll('.ch-nav-item');
        const dropdownMenus = fragmentElement.querySelectorAll('.ch-dropdown-menu');
    ...
    function setupMegaMenuMapping() {
        ...
        dropdownItems.forEach((item, index) => {
            ...
            const allNavItems = Array.from(fragmentElement.querySelectorAll('.ch-nav-item, .ch-nav li, .nav-item'));
            ...
            if (dropdown && megaContent && megaContent.children.length > 0) {
                ...
                megaContentClone.classList.add('ch-mega-content-clone');
                
                // Remove any existing mega content clones first
                const existingClones = dropdown.querySelectorAll('.ch-mega-content-clone');
    ...
    function setupMegaMenuEvents(navItem, megaContent) {
        const dropdown = navItem.querySelector('.ch-dropdown-menu');
        ...
        // Handle click events
        const link = navItem.querySelector('.ch-nav-link');
        if (link) {
            link.addEventListener('click', (e) => {
                e.preventDefault();
